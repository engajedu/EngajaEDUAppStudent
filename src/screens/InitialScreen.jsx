...
export default function InitialScreen({ navigation }) {
  ...
  async function connect() {
    ...
    .then(async response => {
      ...
      try {
        await api.post('/criarColecaoRespostasAluno', {
          matricula: Number(numMatricula)
        });
      } catch (e) {
        console.error('Erro ao criar coleção de respostas:', e.response?.data || e.message);
        Toast.show({
          type: 'error',
          text1: 'Erro ao preparar respostas do aluno'
        });
        return;
    }
